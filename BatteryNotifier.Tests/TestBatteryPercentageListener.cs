...
public class TestBatteryPercentageListener
{
    ...
    [Fact]
    public void TestHighBatteryAlertIsTriggered()
    {
        AutoResetEvent are = new AutoResetEvent(false);

        BatteryInformation battery1 = new BatteryInformation();
        battery1.deviceID = "1";
        battery1.percentage = 85;
        battery1.health = "OK";
        battery1.expectedRunTime = 240;
        battery1.state = BatteryState.Charging;

        IBatteryRepository repository = new InMemoryBatteryRepository();
        repository.Add(battery1);

        _listener = new BatteryPercentageListener(repository);
        _listener.BatteryUpperThresholdReached += (sender, args) => { are.Set(); };
